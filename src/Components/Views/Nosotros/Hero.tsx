...
const Hero = () => {
  return (
    <header className="min-h-screen pt-[88px]">
      <div className="container">
        <h2 className="text-[#4E4B8B] text-[18px] sm:text-[32px] font-bold mb-10">
          Nosotros{" "}
        </h2>
        {text.map((item, index) => {
          return (
              ...
              {item.innerData.map((newItem, idx) => {
                return (
                  <div className="max-w-[618px] mx-auto" key={idx}>
                    <p className="pb-1 text-base  text-center font-bold text-[#3D8095]">
                      {newItem.heading}
                    </p>
                    <p className="pb-5 text-base  text-center font-normal text-[#646467]">
                      {newItem.text}
                    </p>
                  </div>
                );
